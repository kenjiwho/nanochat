...
with open(OUT_FILE, "a", encoding="utf-8") as f:
    for (text_type, text) in all_text:
        ...
        token_seperated = "|".join(tokens)
        f.write(token_seperated)

        token_str = "".join(tokens)

        if token_str != text:
            token_str_orig = tokenizer.decode(encode)
            f.write("\n\n")
            f.write("TOKEN STRING AND ORIGINAL TEXT DID NOT MATCH\n")
            f.write("separate tokens decode:\n")
            f.write(token_str)
            f.write("\n\n")
